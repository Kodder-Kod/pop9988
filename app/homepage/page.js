...
/* ---------------------------------------------------------
   TOKEN FORMULA (demo economics — Kenyan shilling <-> tokens)
   8,000 tokens = KES 100  ->  1 token = KES 0.0125
--------------------------------------------------------- */
const TOKEN_KES_UNIT = 100;    // KES
const TOKEN_UNIT = 8000;       // tokens
const MIN_TOKENS = 8000;       // least amount that can be bought/sold
const SERVICE_FEE_PCT = 2;     // applied on buys only

const tokensToKes = (tokens) => (Number(tokens) || 0) * (TOKEN_KES_UNIT / TOKEN_UNIT);
const kesToTokens = (kes) => (Number(kes) || 0) * (TOKEN_UNIT / TOKEN_KES_UNIT);
...
const platformUsers = [
  { name: "Kevin Otieno", role: "Agent", tokens: 2253 },
...
const AGENT_WALLET_BALANCE = 200000;  // demo balance shown in the agent wallet
const CLIENT_TOKEN_BALANCE = 620;   // demo balance shown on the client identity card
...
function PrimaryButton({ children, onClick, full = true, disabled = false }) {
  return (
    <button
      onClick={onClick}
      disabled={disabled}
      className={`${full ? "w-full" : ""} py-3.5 rounded-2xl bg-gradient-to-r from-purple-700 via-fuchsia-600 to-purple-700 text-white font-bold hover:scale-[1.02] active:scale-95 transition shadow-lg disabled:opacity-40 disabled:hover:scale-100 disabled:cursor-not-allowed`}
    >
      {children}
    </button>
  );
...
// Segmented pill switcher used inside the Wallet modal (Buy / Sell)
function SegmentedTabs({ tabs, active, onChange }) {
  return (
    <div className="bg-purple-50 rounded-full p-1.5 flex gap-1 mb-6">
      {tabs.map((t) => (
        <button
          key={t.id}
          onClick={() => onChange(t.id)}
          className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-full text-sm font-bold transition ${active === t.id ? "bg-white text-purple-700 shadow-sm" : "text-purple-400 hover:text-purple-600"
            }`}
        >
...
// Plain number input styled to match the rest of the modals (used for buy qty,
// and for the two linked fields in the sell flow)
function NumberField({ label, icon: Icon, value, onChange, suffix, min }) {
  return (
    <div className="mb-4">
      <label className="block text-xs font-bold uppercase tracking-wider text-purple-600 mb-2">{label}</label>
      <div className="relative">
        {Icon && <Icon className="absolute left-4 top-1/2 -translate-y-1/2 text-purple-500 w-5 h-5" />}
        <input
          type="number"
          value={value}
          min={min}
          ...
          placeholder="0"
          className={`w-full ${Icon ? "pl-12" : "pl-4"} ${suffix ? "pr-16" : "pr-4"} py-4 rounded-2xl bg-gray-100 border border-transparent focus:border-purple-500 focus:bg-white outline-none transition text-sm text-black font-semibold`}
          style={{ fontFamily: "'JetBrains Mono', monospace" }}
        />
...
/* ---------------------------------------------------------
   BUY TOKENS PANEL — used by both the client "Buy Tokens" modal
   and the agent Wallet modal's Buy tab. Plain number input for
   token quantity; minimum is MIN_TOKENS (8,000 tokens / KES 100).
--------------------------------------------------------- */
function BuyTokensPanel({ qty, setQty, currentBalance, onConfirm }) {
  const [phoneNumber, setPhoneNumber] = useState("");

  const belowMin = Number(qty) < MIN_TOKENS || qty === "";
  const subtotal = tokensToKes(qty);
  const fee = subtotal * (SERVICE_FEE_PCT / 100);
  ...
  return (
    <>
      ...
      <p className={`text-xs -mt-2 mb-4 font-semibold ${belowMin ? "text-rose-500" : "text-gray-400"}`}>
        Minimum purchase is {MIN_TOKENS.toLocaleString()} tokens ({fmtKes(TOKEN_KES_UNIT)}).
      </p>

      <div className="bg-purple-50/70 rounded-2xl p-4 mb-6 space-y-2 text-sm">
        <div className="flex justify-between text-gray-500">
          <span>{Number(qty || 0).toLocaleString()} tokens × {fmtKes(TOKEN_KES_UNIT)}/{TOKEN_UNIT.toLocaleString()}</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace" }}>{fmtKes(subtotal)}</span>
        </div>

        <div className="flex justify-between text-gray-500">
          <span>Service fee ({SERVICE_FEE_PCT}%)</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace" }}>{fmtKes(fee)}</span>
        </div>

        <div className="flex justify-between font-extrabold text-gray-800 pt-2 border-t border-purple-100">
          <span>Total due</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace" }}>{fmtKes(total)}</span>
        </div>
      </div>

      <PrimaryButton
        disabled={belowMin || phoneNumber.trim() === ""}
        onClick={() =>
          onConfirm({
            qty: Number(qty),
            total,
...
/* ---------------------------------------------------------
   SELL TOKENS PANEL — agent only. Two linked inputs: tokens
   and the cash equivalent. Editing either recalculates the other.
--------------------------------------------------------- */
function SellTokensPanel({
  tokenQty,
  cashAmt,
  setTokenQty,
  setCashAmt,
  ...
  const belowMin = Number(tokenQty) < MIN_TOKENS || tokenQty === "";
  const overBalance = Number(tokenQty) > currentBalance;

  const handleTokenChange = (val) => {
    setTokenQty(val);
    setCashAmt(val === "" ? "" : tokensToKes(val).toFixed(2));
  };

  const handleCashChange = (val) => {
    setCashAmt(val);
    setTokenQty(val === "" ? "" : Math.round(kesToTokens(val)));
  };
  ...
  return (
    <>
      ...
      <p
        className={`text-xs mt-2 mb-4 font-semibold ${belowMin || overBalance ? "text-rose-500" : "text-gray-400"
          }`}
      >
        {overBalance
          ? "You don't have enough tokens."
          : `Minimum sale is ${MIN_TOKENS.toLocaleString()} tokens.`}
      </p>

      <div className="bg-emerald-50 rounded-2xl p-4 mb-6 space-y-2 text-sm">
        <div className="flex justify-between">
          <span>Tokens Selling</span>
          <span>{Number(tokenQty || 0).toLocaleString()}</span>
        </div>
        ...
        <div className="flex justify-between border-t pt-2 font-bold">
          ...
          <span>
            {(currentBalance - Number(tokenQty || 0)).toLocaleString()} Tokens
          </span>
        </div>
      </div>
  ...
  return (
    <>
      ...
      <p className={`text-xs -mt-2 mb-4 font-semibold ${belowMin || overBalance ? "text-rose-500" : "text-gray-400"}`}>
        {overBalance
          ? `You only have ${currentBalance.toLocaleString()} tokens available to sell.`
          : `Minimum sale is ${MIN_TOKENS.toLocaleString()} tokens (${fmtKes(TOKEN_KES_UNIT)}). Rate: ${TOKEN_UNIT.toLocaleString()} tokens = ${fmtKes(TOKEN_KES_UNIT)}.`}
      </p>

      <div className="bg-purple-50/70 rounded-2xl p-4 mb-6 space-y-2 text-sm">
        <div className="flex justify-between text-gray-500">
          <span>Tokens sold</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace" }}>{Number(tokenQty || 0).toLocaleString()}</span>
        </div>
        <div className="flex justify-between font-extrabold text-gray-800 pt-2 border-t border-purple-100">
          <span>Cash received</span>
          <span style={{ fontFamily: "'JetBrains Mono', monospace" }}>{fmtKes(cashAmt)}</span>
        </div>
      </div>

      <PrimaryButton
        disabled={belowMin || overBalance}
        onClick={() => onConfirm({ tokenQty: Number(tokenQty), cashAmt: Number(cashAmt), currentBalance })}
      >
        Confirm Sale
      </PrimaryButton>
    </>
...
const Homepage = () => {
  ...
  // Buy Tokens (client modal + agent wallet "Buy" tab)
  const [clientBuyQty, setClientBuyQty] = useState(MIN_TOKENS);
  const [walletBuyQty, setWalletBuyQty] = useState(MIN_TOKENS);

  // Sell Tokens (agent wallet "Sell" tab only)
  const [walletSellTokenQty, setWalletSellTokenQty] = useState(MIN_TOKENS);
  const [walletSellCashAmt, setWalletSellCashAmt] = useState(tokensToKes(MIN_TOKENS).toFixed(2));

  // Which tab is active inside the agent Wallet modal
  ...
  const handleClientBuyConfirm = async ({ qty, total, phoneNumber }) => {
    try {
      ...
      confirmSuccess("M-Pesa Request Sent", [
        ["Phone", phoneNumber],
        ["Tokens", qty.toLocaleString()],
        ["Amount", fmtKes(total)],
        ["Status", "Check your phone and enter your M-Pesa PIN"],
      ]);
  ...
  const handleWalletBuyConfirm = async ({ qty, total, phoneNumber }) => {
    try {
      const res = await fetch("/api/mpesa", {
        method: "POST",
        headers: {
  ...
  const handleWithdrawToMpesa = () => {
    setWalletTab("sell");
  };

  const handleWalletSellConfirm = ({ tokenQty, cashAmt, currentBalance }) => {
    confirmSuccess("Tokens Sold", [
      ["Tokens sold", tokenQty.toLocaleString()],
      ["Cash received", fmtKes(cashAmt)],
      ["New balance", `${(currentBalance - tokenQty).toLocaleString()} tokens`],
    ]);
    setWalletSellTokenQty(MIN_TOKENS);
    setWalletSellCashAmt(tokensToKes(MIN_TOKENS).toFixed(2));
  };
  ...
  return (
    <div className="relative min-h-screen bg-[#f6f6fb] overflow-x-hidden pb-16">
        ...
        <div className="bg-white rounded-[32px] shadow-[0_25px_60px_rgba(124,58,237,0.15)] p-6 sm:p-8 mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {role === "client" && (
            <div className="flex items-center gap-3">
              <div>
                ...
                <TokenChip value={CLIENT_TOKEN_BALANCE} size="lg" />
              </div>
              <button
                onClick={() => { setClientBuyQty(MIN_TOKENS); setModal({ type: "buyTokens" }); }}
          ...
          {role === "agent" && (
            <div className="flex gap-2">
              <button onClick={openWallet} className="flex items-center gap-2 px-4 py-3 rounded-2xl bg-purple-50 text-purple-700 font-bold text-sm hover:bg-purple-100 transition">
          ...
          <>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-4">
              <div className="bg-gradient-to-br from-purple-700 to-fuchsia-600 rounded-3xl p-6 text-white">
                ...
                <p className="text-white/70 text-sm mt-1">≈ {fmtKes(tokensToKes(totalCirculating))} in circulation</p>
              </div>
              <div className="bg-white rounded-3xl p-6 border border-purple-50 shadow-[0_10px_30px_rgba(124,58,237,0.08)]">
                ...
                <p className="text-gray-400 text-sm mt-1">≈ {fmtKes(tokensToKes(TOKENS_SOLD_TO_AGENTS))} issued to agents</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-3 gap-3 sm:gap-4 mb-4">
              ...
              <StatCard icon={PiggyBank} label="Earned" value={fmtKes(earnedValue)} sub="Platform commission" tone="emerald" />
      ...
      {/* Client-only Buy Tokens modal — buy only, no selling */}
      {modal?.type === "buyTokens" && (
        <Modal title="Buy Tokens" subtitle={`${TOKEN_UNIT.toLocaleString()} tokens = ${fmtKes(TOKEN_KES_UNIT)}`} onClose={closeModal}>
          <BuyTokensPanel
            qty={clientBuyQty}
            setQty={setClientBuyQty}
            currentBalance={CLIENT_TOKEN_BALANCE}
            onConfirm={handleClientBuyConfirm}
          />
        </Modal>
      )}
      ...
      {/* Agent Wallet modal — now includes Buy + Sell tabs */}
      {modal?.type === "wallet" && (
        <Modal
          title="Agent Wallet"
          subtitle="Buy or sell tokens, and track your balance"
          onClose={closeModal}
        >
          <div className="bg-gradient-to-br from-purple-700 to-fuchsia-600 rounded-3xl p-6 text-white mb-6">
            ...
            <p
              ...
              style={{ fontFamily: "'JetBrains Mono', monospace" }}
            >
              {AGENT_WALLET_BALANCE.toLocaleString()}
              <span className="text-lg font-semibold"> tokens</span>
            </p>

            <p className="text-white/70 text-sm mt-1">
              ≈ {fmtKes(tokensToKes(AGENT_WALLET_BALANCE))}
            </p>
          </div>

          <SegmentedTabs
            tabs={[
              { id: "buy", label: "Buy Tokens", icon: Plus },
              { id: "sell", label: "Sell Tokens", icon: ArrowUpRight },
            ]}
            active={walletTab}
          ...
          {walletTab === "buy" ? (
            <div className="mt-6">
              <BuyTokensPanel
                qty={walletBuyQty}
                setQty={setWalletBuyQty}
                currentBalance={AGENT_WALLET_BALANCE}
                onConfirm={handleWalletBuyConfirm}
              />
            </div>
          ) : (
            <div className="mt-6">
              <SellTokensPanel
                tokenQty={walletSellTokenQty}
                cashAmt={walletSellCashAmt}
                setTokenQty={setWalletSellTokenQty}
                setCashAmt={setWalletSellCashAmt}
                currentBalance={AGENT_WALLET_BALANCE}
                onConfirm={handleWalletSellConfirm}
              />
            </div>
          )}
        </Modal>
